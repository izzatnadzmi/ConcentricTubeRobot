...
class Controller(object):
    ...
    def __init__(self, UzControl=False, Kp_x=6, Ki_x=0, Kd_x=0, total_time=1, dt=0.05, 
            model=lambda q,uz:moving_CTR(q,uz), 
            jac_del_q=1e-1):
        if UzControl:
            self.model = lambda q,uz:UzController(q,uz).Uz_controlled_model()
        else:
            self.model = model
    ...
    def run(self, a1_c, a2_c, a3_c, q_start, x_end_pos):
        ...
        while i < self.t_steps:
            ...
            r_jac = Jacobian(self.jac_del_q, x, q_des_pos[:, i-1].flatten(), uz_0, self.model)
            J_inv = r_jac.p_inv()
            delta_q[:, i] = J_inv @ (x_des_vel[:, i] + self.Kp_x@(delta_x[:, i]) + 
                                        self.Ki_x@integral + self.Kd_x@(delta_v[:, i]))
            # print('delta_q', delta_q)

            q_des_pos[:, i] = q_des_pos[:, i-1] + delta_q[:, i] * self.dt
            (r1,r2,r3,Uz) = self.model(q_des_pos[:, i], uz_0)        # FORWARD KINEMATICS
            x_cur_pos[:, i] = r1[-1]
...
if __name__ == "__main__":
    ...
    CTR_sim = Controller(UzControl=True, total_time=total_time, dt=dt)
